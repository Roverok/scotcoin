...
// Copyright (c) 2013 Scotcoin developers
...
const string strMessageMagic = "Scotcoin Signed Message:\n";
...
static const int64 nStartSubsidy = .01 * COIN;
static const int64 nMinSubsidy = .01 * COIN;

int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = nStartSubsidy;
    if (nHeight < 101) {nSubsidy = 9800000 * COIN;}
...
static const int64 nTargetTimespan = 12300; 
static const int64 nTargetSpacing = 123;
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 100 blocks
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ScotcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ScotcoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running ScotcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ScotcoinMiner terminated\n");
        throw;
    }
}
